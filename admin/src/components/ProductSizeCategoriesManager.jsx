...
export default function ProductSizeCategoriesManager() {
  ...
  const addSize = (categoryId) => setCategories((items) => items.map((item) => item.id === categoryId ? { ...item, sizes: [...item.sizes, { id: `size-${Date.now()}`, label: 'Size mới', price: '', originalPrice: '', measurements: [{ id: `measurement-${Date.now()}`, label: 'Rộng', value: '', unit: 'cm' }], isActive: true }] } : item));
  ...
  return <div className="panel size-manager nested-size-manager">
    ...
    <div className="size-category-list">{pageItems.map((category) => <section className="size-category-card" key={category.id}>
      ...
      {category.sizes.length ? <div className="table-wrap"><table className="data-table nested-size-table"><thead><tr><th>TÊN SIZE</th><th>GIÁ GỐC</th><th>GIÁ SALE</th><th>THÔNG SỐ KÍCH THƯỚC</th><th>HIỂN THỊ</th><th></th></tr></thead><tbody>{category.sizes.map((size) => <tr key={size.id}><td><input className="table-inline-input size-name-input" value={size.label} onChange={(event) => updateSize(category.id, size.id, 'label', event.target.value)} /></td><td><input className="table-inline-input size-price-input" type="number" min="0" step="1000" value={size.originalPrice ?? ''} placeholder="Giá trước giảm" onChange={(event) => updateSize(category.id, size.id, 'originalPrice', event.target.value === '' ? '' : Number(event.target.value))} /></td><td><input className="table-inline-input size-price-input" type="number" min="0" step="1000" value={size.price ?? ''} placeholder="Giá bán" onChange={(event) => updateSize(category.id, size.id, 'price', event.target.value === '' ? '' : Number(event.target.value))} /></td><td><DimensionInputs size={size} onChange={(key, value) => updateSize(category.id, size.id, key, value)} /></td><td><label className="compact-toggle"><input type="checkbox" checked={size.isActive !== false} onChange={(event) => updateSize(category.id, size.id, 'isActive', event.target.checked)} /><span className="toggle-ui" /></label></td><td><button className="icon-button size-delete-button" onClick={() => setCategories((items) => items.map((item) => item.id === category.id ? { ...item, sizes: item.sizes.filter((entry) => entry.id !== size.id) } : item))}><span className="material-symbols-outlined">delete</span></button></td></tr>)}</tbody></table></div> : <div className="size-empty-state"><span className="material-symbols-outlined">straighten</span><p>Danh mục này chưa có size.</p><button onClick={() => addSize(category.id)}>Thêm size đầu tiên</button></div>}
