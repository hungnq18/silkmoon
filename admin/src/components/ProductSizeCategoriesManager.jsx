...
export default function ProductSizeCategoriesManager() {
  ...
  const addSize = (categoryId) => setCategories((items) => items.map((item) => item.id === categoryId ? { ...item, sizes: [...item.sizes, { id: `size-${Date.now()}`, label: 'Size mới', price: '', measurements: [{ id: `measurement-${Date.now()}`, label: 'Rộng', value: '', unit: 'cm' }], isActive: true }] } : item));
